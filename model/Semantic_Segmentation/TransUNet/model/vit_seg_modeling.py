...
import model.Semantic_Segmentation.TransUNet.model.vit_seg_configs as configs
from model.Semantic_Segmentation.TransUNet.model.vit_seg_modeling_resnet_skip import ResNetV2
...
class Block(nn.Module):
    ...
    def load_from(self, weights, n_block):
        ...
        with torch.no_grad():
            ...
            self.attention_norm.weight.copy_(np2th(weights[os.path.join(ROOT, ATTENTION_NORM, "scale")]))
            self.attention_norm.bias.copy_(np2th(weights[os.path.join(ROOT, ATTENTION_NORM, "bias")]))
            self.ffn_norm.weight.copy_(np2th(weights[os.path.join(ROOT, MLP_NORM, "scale")]))
            self.ffn_norm.bias.copy_(np2th(weights[os.path.join(ROOT, MLP_NORM, "bias")]))
